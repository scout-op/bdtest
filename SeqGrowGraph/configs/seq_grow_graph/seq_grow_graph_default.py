...
model = dict(
    ...
    # --- GCAD: Geom diffusion decoder ---
    geom_decoder_cfg=dict(
        type='SimpleGeomDecoder',
        bev_feat_dim=256,    # will be overridden from head.in_channels at runtime
        ar_hidden_dim=256,   # will be overridden from head.embed_dims at runtime
        model_dim=256,
        coord_dim=2,
        n_steps=1000,
    ),
...
train_pipeline = [
    ...
    dict(type="TransformTopoGeomDualStreams"),
    dict(type="TransformEventsToSeqAndMask"),
    dict(
...
train_dataloader = dict(
    batch_size=18,
    num_workers=10,  # 4
    collate_fn=dict(type="gcad_collate_fn"),
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
