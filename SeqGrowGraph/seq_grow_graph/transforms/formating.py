...
class Pack3DCenterlineInputs(BaseTransform):
    INPUTS_KEYS = ['points', 'img', 'center_seg']
    ...
    def pack_single_results(self, results: dict) -> dict:
        ...
        if 'center_seg' in results:
            center_seg = results['center_seg']
            if center_seg.flags.c_contiguous:
                center_seg = to_tensor(center_seg).contiguous()
            else:
                center_seg = to_tensor(np.ascontiguousarray(center_seg))
            results['center_seg'] = center_seg
        ...
        # always include dual-stream metas if present
        for extra_key in ['gt_nodes_idx', 'gt_nodes_xy', 'gt_edges', 'gt_geom_blocks',
                          'gt_topo_seq', 'gt_logits_mask', 'gt_add_node_indices']:
            if extra_key in results:
                data_metas[extra_key] = results[extra_key]
