...
class NuScenesReachMetric(BaseMetric):
    ...
    def _format_line(self, results, jsonfile_prefix=None):
        ...
        for sample_id, res in enumerate(mmengine.track_iter_progress(results)):
            ...
            # 同时保存序列和 Diffusion 坐标（如果有）
            result_entry = {
                'sequence': node_list,
            }
            if 'pred_nodes_xy' in res['line_results']:
                result_entry['nodes_xy'] = res['line_results']['pred_nodes_xy'].tolist()
                result_entry['nodes_mask'] = res['line_results']['nodes_mask'].tolist()
            
            nusc_annos[sample_token] = result_entry
