...
from projects.SeqGrowGraph.seq_grow_graph.debug_utils import debug_print
...
# Backward compatibility: configs still reference the legacy name.
# Registering a thin alias keeps older configs functional without duplication.
@TRANSFORMS.register_module()
class OrgLoadMultiViewImageFromFiles(CustomLoadMultiViewImageFromFiles):
    """Alias of `CustomLoadMultiViewImageFromFiles` for legacy configs."""
    pass

@TRANSFORMS.register_module()
class TransformTopoGeomDualStreams(object):
    """解析 `centerline_sequence` 为拓扑事件与几何块，兼容两种编码格式：

    1) 带分隔符/偏移编码（TransformGraph2Seq: 使用 idx_start/coeff_start 与 split_connect/split_node），
       形如 [x,y, idx_token, (父: idx_token, coeff...,)*, split_connect, (子: idx_token, coeff...,)*, split_node, ...]
    2) 固定子句扁平编码（TransformOrderedBzLane2Graph: 子句=4+2*(n_control-2)，[x,y,cls,ptr,coeff...,...]）。

    输出统一为：
      - `gt_topo_events`: list[dict{node:int, xy:[x,y], in_ptrs:List[int], out_ptrs:List[int]}]
      - `gt_geom_blocks`: list[dict{src:int, dst:int, coeff:np.ndarray}]
    """

    def __init__(self, idx_start: int = 250, coeff_start: int = 350,
                 split_connect: int = 571, split_node: int = 572):
    ...
    def __call__(self, results):
        seq = results.get('centerline_sequence', None)
        if seq is None:
            results['gt_topo_events'] = []
            results['gt_geom_blocks'] = []
            debug_print('xform/dual', "no centerline_sequence; emit empty events/blocks")
            return results

        if isinstance(seq, np.ndarray):
            seq = seq.tolist()

        n_control = int(results.get('n_control', 3))
        k = 2 * max(n_control - 2, 0)

        if any(tok == self.split_connect or tok == self.split_node for tok in seq):
            topo_events, geom_blocks = self._parse_delimited(seq, k)
        else:
            topo_events, geom_blocks = self._parse_flat(seq, k)

        # 辅助派生：节点顺序与边集合
        nodes_xy = np.array([e['xy'] for e in topo_events], dtype=int) if len(topo_events) else np.zeros((0, 2), dtype=int)
        nodes_idx = list(range(len(topo_events)))
        edges = []
        ...
        results['gt_topo_events'] = topo_events
        results['gt_geom_blocks'] = geom_blocks
        results['gt_nodes_idx'] = nodes_idx
        results['gt_nodes_xy'] = nodes_xy
        results['gt_edges'] = edges
        debug_print('xform/dual', lambda: (
            f"seq_len={len(seq)}; events={len(topo_events)}; blocks={len(geom_blocks)}; "
            f"nodes_xy={nodes_xy.shape}; edges={len(edges)}"
        ))
        return results

    def _parse_delimited(self, seq, k: int):
        L = len(seq)
        i = 0
        node_id = 0
        topo_events = []
        ...
        while i < L:
            if i + 2 >= L:
                break
            x = int(seq[i]); y = int(seq[i + 1]); idx_token = int(seq[i + 2])
            j = i + 3
            in_ptrs = []
            out_ptrs = []

            # 父边列表（直到 split_connect 或 split_node）
            while j < L and int(seq[j]) not in (self.split_connect, self.split_node):
                src_token = int(seq[j]); src_idx = src_token - self.idx_start
                j += 1
                coeff = np.array(seq[j:j + k], dtype=int) - self.coeff_start if k > 0 else np.zeros((0,), dtype=int)
                j += k
                in_ptrs.append(src_idx)
                if k > 0 and coeff.size > 0:
                    geom_blocks.append({'src': src_idx, 'dst': node_id, 'coeff': coeff})

            if j < L and int(seq[j]) == self.split_connect:
                j += 1

            # 子边列表（直到 split_node）
            while j < L and int(seq[j]) != self.split_node:
                dst_token = int(seq[j]); dst_idx = dst_token - self.idx_start
                j += 1
                coeff = np.array(seq[j:j + k], dtype=int) - self.coeff_start if k > 0 else np.zeros((0,), dtype=int)
                j += k
                out_ptrs.append(dst_idx)
                if k > 0 and coeff.size > 0:
                    geom_blocks.append({'src': node_id, 'dst': dst_idx, 'coeff': coeff})

            if j < L and int(seq[j]) == self.split_node:
                j += 1

            topo_events.append({'node': node_id, 'xy': [x, y], 'in_ptrs': in_ptrs, 'out_ptrs': out_ptrs})
            node_id += 1
            i = j

        return topo_events, geom_blocks

    def _parse_flat(self, seq, k: int):
        clause = 4 + k
        N = len(seq) // clause
        seq = seq[:N * clause]
        topo_events = []
        ...
        for i in range(N):
            base = i * clause
            x = int(seq[base + 0]); y = int(seq[base + 1])
            cls_id = int(seq[base + 2]); ptr_raw = int(seq[base + 3])
            coeff = np.array(seq[base + 4: base + 4 + k], dtype=int) if k > 0 else np.zeros((0,), dtype=int)
            ...
            elif cls_id in (2, 3):  # fork / merge
                src = (ptr_raw - 1) if ptr_raw > 0 else None

            if src is not None:
                in_ptrs.append(src)
                if k > 0 and np.any(coeff):
                    geom_blocks.append({'src': src, 'dst': i, 'coeff': coeff})

            topo_events.append({'node': i, 'xy': [x, y], 'in_ptrs': in_ptrs, 'out_ptrs': out_ptrs})
