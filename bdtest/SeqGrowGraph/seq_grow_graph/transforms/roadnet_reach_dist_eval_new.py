    ...
    for si in range(dataset_range[0], dataset_range[1]):
        try:
            ...
            if token in results:
                ...
                # 解析预测数据（新格式：dict with sequence + coords，旧格式：直接序列）
                pred_result = results[token]
                if isinstance(pred_result, dict):
                    pred_sequence = pred_result['sequence']
                    external_nodes_xy = np.array(pred_result.get('nodes_xy', None))
                    nodes_mask = np.array(pred_result.get('nodes_mask', None))
                else:
                    # 旧格式向后兼容
                    pred_sequence = pred_result
                    external_nodes_xy = None
                    nodes_mask = None
                ...
                pred_nodegraph = EvalSeq2Graph(
                    ...
                    bz_pc_range=bz_pc_range,
                    bz_dx=bz_dx,
                    is_junction_only=is_junction_only,
                    external_nodes_xy=external_nodes_xy,  # 传递 Diffusion 坐标
