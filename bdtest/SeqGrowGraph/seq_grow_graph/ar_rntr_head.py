...
class ARRNTRHead(AnchorFreeHead):
    ...
    def forward(self, mlvl_feats, input_seqs, img_metas, return_hidden: bool = False, logits_mask: torch.Tensor = None):
        """Forward function.
        Args:
        ...
        if self.training:
            ...
            out = self.vocab_embed(outs_dec)  # [6, B, L, V]
            # optional validity masking on logits (training only)
            if logits_mask is not None:
                # logits_mask expected shape [B, L, V] or [1, B, L, V] / [nb_dec, B, L, V]
                if logits_mask.dim() == 3:
                    mask = logits_mask.unsqueeze(0)  # broadcast over decoder layers
                else:
                    mask = logits_mask
                # ensure device match
                mask = mask.to(out.device)
                out = out.masked_fill(~mask, -1e9)
            return (out, outs_dec) if return_hidden else out
        else:
            values = []
            i = 0
            last_full_outs_dec = None  # 保存最后一次完整的 decoder 输出
            ...
            while i < self.max_iteration:
                ...
                # 运行 Transformer，获取完整的 decoder 输出
                outs_dec, _ = self.transformer(tgt, x, masks, query_embed, pos_embed)
                outs_dec = torch.nan_to_num(outs_dec)
                
                # 保存完整的最后一层 decoder 输出 [B, L, C]
                last_full_outs_dec = outs_dec[-1]  # 取最后一层
                
                # 只取最后一个 token 用于生成下一个 token
                current_step_out = outs_dec[-1, :, -1, :]  # [B, C]
                out = self.vocab_embed(current_step_out)  # [B, V]
                out = out.softmax(-1)
            ...
            # 根据 return_hidden 参数决定返回值
            if return_hidden:
                # 返回: (input_seqs, values, hidden_states)
                # hidden_states: [B, L, C] 包含了整个生成序列的特征
                return input_seqs, values, last_full_outs_dec
            else:
