...
class EvalSeq2Graph_with_start(EvalSeq2Graph):
    def __init__(self, map_token, seq,pc_range, dx, bz_pc_range, bz_dx, bezier_keys=50, pixels_step=1, use_pixels=False,front_camera_only=False,is_gt_fix=False,is_junction_only=False, external_nodes_xy=None, nodes_mask=None):
        """
        Args:
            external_nodes_xy: Optional [N, 2] array of node coordinates (Grid coords). 
                              If provided, overrides coordinates parsed from sequence.
            nodes_mask: Optional [N] bool array indicating valid nodes.
        """
        self.token = map_token
        self.roots = []
        self.bezier_keys = bezier_keys
        ...
        for i in range(len(split_node_idxs)-1):
            ...
            # 解析节点索引
            node_index = node_seq[2] - self.idx_start
            
            # 优先使用外部提供的坐标（来自 Diffusion）
            if external_nodes_xy is not None and nodes_mask is not None:
                if node_index < len(external_nodes_xy) and nodes_mask[node_index]:
                    coord = external_nodes_xy[node_index].astype(int).tolist()
                else:
                    # 如果外部坐标不可用，从序列解析
                    coord = [node_seq[0], node_seq[1]]
            else:
